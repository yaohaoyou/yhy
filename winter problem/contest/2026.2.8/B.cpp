...
// void dfs(int u){
//     col[u]=0;while(ch[u][col[u]])col[u]++;
//     go(u,i) ch[t].set(col[u]);
//     go(u,i)if(!~col[t])dfs(t);
// }
void dfss(int x){
    if(x>tot){
        puts("yes");
        for(int i=1;i<=n;i++,puts(""))for(int j=1;j<=m;j++)printf("%d ",col[id[i][j]]);
        exit(0);
    }
    for(int i:{0,1,2,3}){
        col[x]=i;
...
int main(){
    ...
    for(int i=1;i<=n;i++)for(int j=1;j<=m;j++)  printf("(%d,%d)\n",i+j,i-j);
    U.init();
    for(int i=1;i<=n;i++){
        for(int j=1;j<=m;j++){
            for(int p=1;p<=n;p++){
    ...
    mems(col,-1);
    while(count(col+1,col+tot+1,-1)){
        for(int u=1;u<=tot;u++){
            if(~col[u]) continue;
            bool flag=true;
            go(u,i)if(col[t]==cur){flag=false;break;}
            if(flag)col[u]=cur;
        }
        cur++;
    }
