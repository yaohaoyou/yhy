...
int main(){
    ...
    // T=read();
    // bool flag=true;
    // for(int i=1;i<=T;i++){n=read();m=read();Q[m].eb(n,i);qq[i]=pii(n,m);flag&=!(m&1);}
    // if(flag)    return mdouble_solve(),0;
    for(m=1;m<=10000;m++){
        if(m%2==0)  continue;
        if(m>=2500){f[m*2]=0;continue;}
        n=10000;
        mems(f,-1);
        f[m+m]=m;
        for(int i=m+1;i<=n;i++){
        ...
        printf("{");for(int i=m+1;i<=n;i++)if(!((~f[i])&&f[i]))printf("%d,",i);printf("},");
    }
    for(int i=1;i<=T;i++){
        puts(ans[i]?"YES":"NO");
        // if(!ans[i]) printf("%d %d\n",qq[i].fi,qq[i].se);
