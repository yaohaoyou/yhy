...
namespace SegmentTree{
    ...
    ll tr[maxn<<2],tag[maxn<<2],asl[maxn<<2],asr[maxn<<2];
    int L[maxn<<2],R[maxn<<2],mxp[maxn<<2];
    ...
    void clear(){mems(tr,0);mems(tag,0);mems(asl,0);mems(asr,0);mems(L,0);mems(R,0);mems(mxp,0);}
}
