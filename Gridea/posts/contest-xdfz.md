...
不难发现 2 操作前原本的 $L_i$ 就是 1 操作时移除的，所以可以尝试将 2 操作和 1 操作放在一起做。具体的，可以维护 $mid-l+1$ 个``vector``，其中 $vec_i=\{j|L_j=i\}$，操作 1,2 就是将所有 $b_i<x$ 的 $vec_{b_i}$ 合并至 $vec_x$ 并重新计算贡献，同时记录 $s_i=\sum_{j\in vec_i} a_{c_j}\times(c_{j+1}-c_j)$，贡献就是 $s_i\times (mid-i)$。由于 3 操作中更新 $c_2$ 的贡献和 1 操作的删除贡献需要重新查询 $L_i$，但查询前驱直接做至少要单 $\log$，所以应该在 1,2 操作时直接在合并时更新 $L_i$。实际上，动态维护 $L_i$ 只需要支持合并和单点查询，所以不需要使用 ``vector``，直接用并查集就可以做到单次查询 $\mathcal{O}(\alpha(n))$。

至此，我们已经完成了 3 种操作，对于 $x\in (mid,r]$ 的情况类似。总复杂度为 $\mathcal{O}(n\log n\alpha(n))$。