...
namespace DFS{
    int w[9][maxn],f[9][maxn],dep[maxn];
    ...
    inline void init_LCA(){for(int i=1;i<=8;i++)for(int j=1;j<=n;j++){w[i][j]=min(w[i-1][j],w[i-1][f[i-1][j]]);f[i][j]=f[i-1][f[i-1][j]];}}
