...
using namespace std;
const int maxn=210,maxp=maxn*maxn,inf=0x3f3f3f3f,dir[8][2]={{3,1},{-3,1},{3,-1},{-3,-1},{1,3},{1,-3},{-1,3},{-1,-3}};
int n,m,k;
int a[maxn][maxn];
namespace Graph{
...
int main(){
    ...
    for(int i=1;i<=n;i++)
        for(int j=1;j<=m;j++){
            sum+=a[i][j];
            id[i][j]=++tot;
            if(i&1){adde(S,id[i][j],a[i][j]);adde(id[i][j],T,0);}
