...
将询问挂在线段树上离线，这样即使对每个区间内的所有数操作也只有 $\OO(n\log n)$。要求一个点和多个点的分别的 lca，考虑先对每个线段树区间和插入其中的询问节点建出虚树，此时所有的 $lca(a_k,u)$ 都会在虚树上出现。$\dia$
...
总复杂度为 $\OO((n+m+q)\log^2 n)$，瓶颈在给所有区间建虚树。如果用单调栈建虚树，那一次排序可以在线段树上归并处理，复杂度少一个 $\log$。注意要用 $\OO(1)$ lca。
...
$$
f(l,r)=\sum_{i=l}^r|\opn{lcp}(s[l,r],s[i,r])|=\sum_{i=l}^r|\min(\opn{lcp}(s[l,n],s[i,n]),r-i+1)|
$$
由于 $\opn{lcp}(s[i,n],s[j,n])=\min_{k=rk_{i}+1}^{rk_{j}}ht_k$，所以考虑将 $i\in [l,r]$ 分成 $rk_i< rk_l$ 和 $rk_i>rk_l$ 和 $i=l$，然后将询问挂在 $rk_l$ 上。后面其实就是数据结构的事了。
...
由于每次询问只会删除一个点，所以可以考虑缺一分治。$\dia$同时由于 $n\le 300$，所以考虑用 floyd 做最短路。具体的令 $solve(l,r)$ 表示进入时 $[1,l)$ 和 $(r,n]$ 都已经加入图中并处理完最短路，则答案直接离线后在 $solve(p_i,p_i)$ 中求即可。每次加入一段区间 $[l,r]$ 只要将其设为 floyd 中的转移点即可，因为不是它作为转移点的在之前已经处理过了，大概过程就是 $i\in[l,r],j\in [1,n],k\in [1,n] :f_{j,k}\gets f_{j,i}+f_{j,k}$，这个过程单次复杂度为 $\OO(n^2siz)$，由于在分治过程中 $\sum siz=\OO(n\log n)$，所以总复杂度为 $\OO(n^3\log n+q)$。由于 floyd 无法删除，撤销只能记录当时的最短路数组后拷贝，所以空间复杂度是 $\OO(n^3)$。
...
对于一条边 $(u,v)$ 会在 $maxc\in [\max(c_u,c_v),\min(t_u,t_v)]$ 时称为 1 类边，在 $maxc\in [c_u,\min(t_u,c_v-1)]$ 时成为 2 类边，注意反向边也要建。考虑如何顺着这个图进行转移，由于连通块之间的信息是相同的，而且不好撤销，而一条边会在一个区间出现，所以套路地使用线段树分治，1 类连通块用可撤销并查集维护。复杂度为 $\OO(n\log^2n)$。
...
第二个式子是因为考虑 $\binom Fi$ 的组合意义就是在所有的奇数编号球中选择 $i$ 个的方案数，所以可以先钦定被选择的 $i$ 个位置选奇数，然后剩下的随便选。代码实现时将 $i!\binom ni$ 转成了 $n^{\underline i}$ 求解，复杂度 $\OO(Tk^2)$。枚举 $i$ 时动态维护 $n^{\underline i}$ 做到 $\OO(Tk)$。

## [P12999 [GCJ 2022 #3] Mascot Maze](https://www.luogu.com.cn/problem/P12999)

神秘困难图染色构造题，这个 $|S|=13$ 甚至有用。

先把限制 $ans_u\neq ans_v$ 的边 $(u,v)$ 先建出来，无向边数只有 $6n$ 条，若有自环则显然无解。考虑稀疏图的性质，由于有向边数只有 $12n$，而一共只有 $n$ 个点，由鸽巢原理得至少有一个点的度数 $\le 12\dia$，否则至少会有 $13n$ 条有向边，矛盾。然后删除一个点后一定还有至少一个点度数 $\le 12$，所以可以考虑做一个拓扑排序类似的事情。取到拓扑序后倒着染色，每个点此时至多会有 $12$ 条出度，但是有 $13$ 种颜色，所以可以染上。

## [P7220 [JOISC 2020] 掃除](https://www.luogu.com.cn/problem/P7220)

考虑将一个在二维上的点 $(x,y)$ 转化成一维上的一段区间 $[x,n-y]$ $\dia$，由于所有的点在直角三角形中所以一定有 $x+y\le n$。然后发现 H 操作就是将 $r\ge n-L,l\gets \max(l,n-L)$，V 操作就是将 $l\le L,r\gets \min(r,L)$。考虑建立猫树（线段树）后将所有区间挂上去，下面只考虑操作 H 对于 $r\ge x,l\gets \max(l,x)$。

若 $x\le mid$，则需要将所有区间按照 $l$ 排序后取一段前缀推平成 $x$，直接维护 $l$ 相同的连续段即可。

若 $x>mid$，则所有当前线段树节点上的区间都会被移动到右儿子，这个过程对于每个区间显然只会出现 $\OO(\log n)$ 次，所以直接暴力将当前节点的区间取出后全部重新插入即可。

维护这个过程应该直接对每个线段树节点都使用堆和 odt 维护就可以了。总复杂度为 $\OO((n+q)\log^2n)$，难写+难调+速度慢+空间大。
...
$$
n\gets |A|,m\gets |B|\\
\max_{i=1}^P \opn{LCP}(A[i,P],B[m-S+1,m]) \\
=\max_{i=1}^P \min(P-i+1,\opn{LCP}(A[i,n],B[m-S+1,m])) \\
$$
求两个串后缀的 LCP，套路地对串 ``A+#+B`` 做 SA$\dia$，则答案就变成了
$$
\max_{i=1}^P\min(P-i+1,\min_{j=rk_i+1}^{rk_{len-S+1}} ht_j)
$$
后面的 $\min$ 实际上是上下界可能会相反，大概意思知道就行了。

这个区间 $\min$ 还是不太好求，考虑将 $rk_i$ 排序后建立对应的 $ht$ 的小根笛卡尔树，然后就将区间 $\min$ 转化成求笛卡尔树上的 LCA$\dia$。考虑枚举 $rk_{len-Q+1}$ 的 LCA 为 $u$，则对答案贡献 $\min(P-mn_u+1,ht_u)$，其中 $mn_u$ 表示 $u$ 子树内的编号最小值。发现 $u$ 越往上 $mn_u$ 越小，$P-mn_u+1$ 越大，$ht_u$ 越小，所以可以在链上二分求解两个函数交点就是答案。复杂度 $\OO(\sum (n+m+q)\log (n+m))$。
...
$$
由于第二维的大小为 $\OO(n^2)$ 的，所有直接做复杂度是 $\OO(n^6)$，即使用 FFT 也只能做到 $\OO(n^4\log n)$。

看到第二维是卷积形式，考虑对序列 $f_i$ 做生成函数$\dia$。令 $F_i(x)=\sum_{j=i}^{n^2}f_{i,j} x^j$，则转移式可以写成
$$
F_i=\sum_{k=0}^{i-1}\binom{i-1}k x^i F_kF_{i-1-k}
$$
由于 $F_i$ 是一个至多 $n^2$ 次的多项式，所以只要求出 $\OO(n^2)$ 个点值后拉差即可复原系数。$\dia$对于每个 $i$ 维护 $g_{i,j}=F_i(j)$，卷积时直接对位乘即可，最后要给 $n^2$ 次多项式做拉差复杂度为 $\OO((n^2)^2)=\OO(n^4)$。

这题能对 FFT 加速的主要原因是需要做 $\OO(n^2)$ 次卷积，若只维护点值单次做卷积的复杂度就是 $\OO(n^2)$，最后在拉差还原时也是 $\OO(n^4)$ 的。若只做一次，拉差还原部分就已经是 $\OO(n^2)$ 的，不如暴力。

#### 已知 $k$ 次多项式的 $k+1$ 个点值，$\OO(k^2)$还原多项式。
...
\end{align}
$$
前半部分的值是已知的，直接 $\OO(k^2)$ 枚举 $i,j$ 求即可，后半部分就是多个一次函数相乘，直接每次暴力展开是 $\OO(k^3)$ 的，但是可以写成 $\frac{\prod(x-x_j)}{x-x_i}$，然后预处理 $\prod(x-x_i)$ 的系数后每次做大除法即可，复杂度为 $\OO(k^2)$。
