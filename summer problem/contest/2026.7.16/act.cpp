...
int main(){
    ...
    //     auto [i,j]=*it;
    //     if ((sx.find(i)!=sx.end()&&sx[i]<j)||(sy.find(j)!=sy.end()&&sy[j]<i)){
    //         st.erase(it);
    //         it=st.insert({i,-1}).fi;
    //     }
    // }
    ...
    for(int _cases=1;_cases<=q;_cases++){
        ...
        if(z.fi==x&&z.se==y&&z.id==0) pus("Bob\n");
        else if((::sx.find(x)!=::sx.end()&&::sx[x]<y)||(::sy.find(y)!=::sy.end()&&::sy[y]<x))    pus("Alice\n");
        else pus(ans[_cases]?"Alice\n":"Bob\n");
    }
    FastIO::flush();
}