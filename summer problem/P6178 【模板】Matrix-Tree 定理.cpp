...
int main(){
    scanf("%d%d%d",&n,&m,&op);
    iota(id+1,id+n+1,1);random_shuffle(id+1,id+n+1);
    for(int i=1,u,v,w;i<=m;i++){
        scanf("%d%d%d",&u,&v,&w);
        u=id[u];v=id[v];
        // u--;v--;
        if(!op){
            madd(a[u][u],w);madd(a[v][v],w);
            mdel(a[u][v],w);mdel(a[v][u],w);
        }
        else{madd(a[v][v],w);mdel(a[u][v],w);}
    }
    printf("%d",calc());
}