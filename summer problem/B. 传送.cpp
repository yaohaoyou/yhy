...
int L[maxn],R[maxn],rk[maxn],fa[maxn];
...
frac mp[maxn];int vv[maxn],vl;
inline frac calc(int x,int k){
    if(!(mp[k]==frac()))   return mp[k];
    vv[++vl]=k;
...
vector<pli> teleport(int c,int N,int M,vector<int> u,vector<int> v,vector<int> x,vector<int> y){
    ...
    for(int u=1;u<=n;u++){
        ...
        for(int i=1;i<=vl;i++)  mp[vv[i]]=frac();vl=0;
        int l=0,r=0;frac mn;
        ...
        for(auto [s,id]:Q[u])ans[id]=mn<frac(dis(s,u))?pli(mn.p,mn.q):pli(dis(s,u),1);
    }
    return ans;
}
