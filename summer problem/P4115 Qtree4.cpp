...
namespace FastIO{
    ...
    inline void putstr(string s){for(char i:s)putc(i);}
}
using FastIO::read;using FastIO::readll;using FastIO::print;using FastIO::putstr;using FastIO::getc;using FastIO::putc;using FastIO::flush;
namespace Graph{
...
struct my_multiset{
    ...
    inline int ts(){int x=top();q1.pop();int y=top();q1.ep(x);return x+y;}
};
...
inline void upd(int x){if(B[x].size()<2) return;A.ep(B[x].ts());}
inline void del(int x){if(B[x].size()<2) return;A.erase(B[x].ts());}
int ctt;
inline void lightup(int u){
    ...
    while(fa[u]){
        ...
        int bf=C[u].empty()?-1e9:C[u].top();
        C[u].ep(dis(fa[u],x));
        int nw=C[u].top();
        if(bf^nw){if(bf!=-1e9)B[fa[u]].erase(bf);B[fa[u]].ep(nw);}
...
inline void lightdown(int u){
    ...
    while(fa[u]){
        ...
        int nw=C[u].empty()?-1e9:C[u].top();
        if(bf^nw){B[fa[u]].erase(bf);if(nw!=-1e9)B[fa[u]].ep(nw);}
        // if(!C[u].empty())  B[fa[u]].ep(C[u].top());
        upd(fa[u]);
        u=fa[u];
    }
...
int main(){
    n=read();
    for(int i=1;i<n;i++){int u=read(),v=read();adde(u,v,read());}
    ST_LCA::build();
    ...
    while(q--){
        ...
        if(c=='C'){
            ...
            if(A.empty()&&!ctt)     putstr("They have disappeared.\n");
            else    print(ctt==1?0:max(0,A.top())),putc('\n');
        }
